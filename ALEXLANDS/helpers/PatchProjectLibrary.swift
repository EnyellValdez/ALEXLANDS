...
enum PatchProjectLibrary {
    ...
    static func load(fileManager: FileManager = .default) -> [PatchLibraryItem] {
        ...
        for url in urls where PatchPackageFile.accepts(url) {
            do {
    ...
    static func exportURL(for packageURL: URL, fileManager: FileManager = .default) throws -> URL {
        guard packageURL.pathExtension.lowercased() != PatchPackageFile.fileExtension else {
            return packageURL
        }
        let directory = fileManager.temporaryDirectory
            .appendingPathComponent("EnyellTS-Exports", isDirectory: true)
            .appendingPathComponent(UUID().uuidString, isDirectory: true)
        try fileManager.createDirectory(at: directory, withIntermediateDirectories: true)
        let destination = directory
            .appendingPathComponent(packageURL.deletingPathExtension().lastPathComponent)
            .appendingPathExtension(PatchPackageFile.fileExtension)
        do {
            // Copy bytes verbatim: encryption, password and package identity stay intact.
    ...
    static func removeExportCopy(at url: URL, fileManager: FileManager = .default) {
        let directory = url.deletingLastPathComponent().standardizedFileURL
        let exportRoot = fileManager.temporaryDirectory
            .appendingPathComponent("EnyellTS-Exports", isDirectory: true).standardizedFileURL
        guard directory.deletingLastPathComponent() == exportRoot else { return }
        try? fileManager.removeItem(at: directory)
    }
    ...
    static func save(
        ...
        if let existingURL {
            ...
            var candidate = root.appendingPathComponent(baseName).appendingPathExtension(PatchPackageFile.fileExtension)
            var suffix = 2
            while fileManager.fileExists(atPath: candidate.path) {
                candidate = root.appendingPathComponent("\(baseName)-\(suffix)").appendingPathExtension(PatchPackageFile.fileExtension)
                suffix += 1
            }
