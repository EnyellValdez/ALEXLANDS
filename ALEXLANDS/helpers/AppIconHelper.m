    ...
    dispatch_once(&onceToken, ^{
        ...
        for (size_t i = 0; i < sizeof(candidates) / sizeof(candidates[0]); i++) {
            if (dlopen(candidates[i], RTLD_LAZY | RTLD_GLOBAL)) {
                NSLog(@"[ALEXLANDS] ls: loaded %s", candidates[i]);
                return;
            }
        }
        NSLog(@"[ALEXLANDS] ls: CoreServices/MobileCoreServices dlopen failed");
    });
}
...
static NSDictionary *appsFromWorkspace(void) {
    ...
    for (NSString *selectorName in @[@"allApplications", @"allInstalledApplications"]) {
        ...
        NSLog(@"[ALEXLANDS] ls: %@ returned %lu", selectorName,
              (unsigned long)([candidate isKindOfClass:[NSArray class]] ? [candidate count] : 0));
    }
    if (!apps || apps.count == 0) {
        NSLog(@"[ALEXLANDS] ls: workspace enumeration empty");
        return result;
    }
    NSLog(@"[ALEXLANDS] ls: %@ returned %lu proxies", usedSelector, (unsigned long)apps.count);
    ...
    NSLog(@"[ALEXLANDS] ls: extracted %lu apps (%lu with container)",
...
NSDictionary<NSString *, NSDictionary *> *installedAppInfo(void) {
    ...
    NSLog(@"[ALEXLANDS] ls: workspace empty; MobileInstallation=%lu",
