...
private enum PatchPackagePickerPolicy {
    static let packageType = UTType(filenameExtension: PatchPackageFile.fileExtension) ?? .data
...
private struct PatchProjectDetailView: View {
    ...
    var body: some View {
        ...
        .sheet(item: $shareRequest, onDismiss: {
            if let exportedURL {
                PatchProjectLibrary.removeExportCopy(at: exportedURL)
    ...
    private func prepareExport() {
        ...
        Task.detached(priority: .userInitiated) {
            do {
                ...
                let url = try PatchProjectLibrary.exportURL(for: item.packageURL)
                await MainActor.run {
                    store.reload()
                    isWorking = false
                    exportedURL = url
                    shareRequest = PatchShareRequest(url: url)
                }
            } catch let error as PatchPackageError {
                await MainActor.run {
                    isWorking = false
