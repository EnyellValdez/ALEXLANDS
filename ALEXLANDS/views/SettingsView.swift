            ...
            Form {
                Section {
                    HStack(spacing: 14) {
                        ...
                        VStack(alignment: .leading, spacing: 3) {
                            Text("ALEXLANDS").font(.headline)
                            Text(language.text("common.version", appVersion))
                                .font(.subheadline)
                                .foregroundStyle(.secondary)
                            Text(language.text("settings.build", Bundle.main.object(forInfoDictionaryKey: "CFBundleVersion") as? String ?? "—"))
                                .font(.caption)
                                .foregroundStyle(.secondary)
                        }
                    }
                ...
                Section {
                    LabeledContent {
                        ...
                        Label(language.text("settings.authorized_user"), systemImage: "person.crop.circle.badge.checkmark")
                    }
                    
                    LabeledContent {
                        Text(language.text("settings.ready"))
                            .fontWeight(.bold)
                            .foregroundStyle(.green)
                    } label: {
                        Label(language.text("settings.status"), systemImage: "shield.righthalf.filled")
                    }
                }
                .listRowBackground(Color.black.opacity(0.3).background(Material.ultraThinMaterial))
